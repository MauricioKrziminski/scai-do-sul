...
A área segura do aparelho também entra na conta. O site declara `viewportFit: "cover"`,
então desenha até a borda física da tela e o cabeçalho carrega o recorte como
respiro de cima. A altura descontada é `5.0625rem + env(safe-area-inset-top)`, e não
só o número fixo. Simulado com recorte de 59px em 6 rotas: cabeçalho mais faixa
somam exatamente a tela.
...
| 2026-08-24 | Toda rota ganhou o seu `opengraph-image` | Só a home e as linhas de produto tinham prévia de link. As outras cinco rotas declaram `openGraph` na própria metadata para fixar a `url` canônica, e o Next substitui o bloco inteiro em vez de mesclar, então a imagem da raiz não descia. Compartilhar /empresa, /produtos, /setores, /contato ou /privacidade no WhatsApp dava prévia sem imagem. Cada rota agora tem a sua, com o título igual ao da página |
...
| 2026-08-24 | Site passou a declarar `viewportFit: "cover"`, com `env(safe-area-inset-*)` no cabeçalho, no painel do menu, no botão do WhatsApp e na margem do Container | No Safari do iPhone 17 Pro o conteúdo da página desenha de ponta a ponta, por baixo da barra de status, mas `top: 0` e `bottom: 0` de elemento fixo resolviam dentro da área segura. A barra grudada sentava uns 60px abaixo do topo real e o painel do menu terminava uns 60px acima do pé real, e nas duas faixas aparecia a página passando atrás. Declarar `cover` alinha as duas coisas, e aí é o site que afasta o conteúdo do recorte. Em aparelho sem recorte todo `env` vale zero e nada muda. Só que `cover` sozinho não resolveu no aparelho do cliente, então as duas faixas vão cobertas na mão: um bloco de papel sai por cima da barra grudada, e outro sai por baixo do painel do menu enquanto ele está aberto. Só enquanto aberto, porque com o menu fechado quem está no pé é o rodapé escuro, e uma barra de papel por cima dele seria pior que o vão |
